...
int main()
{
	...
	data_pembeli();
	lambang (120);
	printf("%c-----------------------------------------WELCOME TO MALANG LAPTOP SHOP------------------------------------------------%c",177,177); Sleep(300);
	lambang (120);Sleep(250);lambang(120);
	printf ("\t\t\t\t\t This Application Created By:\n");
	printf ("\t\t\t\t\t   1.Ilham Bagus Masnanda\n");Sleep(100);
	printf ("\t\t\t\t\t    2.Lucas Marcellino\n");Sleep(100);
	printf ("\t\t\t\t\t     3.Wibrian Alfathan\n");Sleep(100);
	system("pause");
	menu();
}
...
{
	FILE *pmb;
    pmb=fopen("DataCustomers.txt","a");
	while (fscanf(pmb,"%s %s %s %d",&customer[indeks].nama, &customer[indeks].alamat, &customer[indeks].barangdibeli , &customer[indeks].jmlhbrg)!=EOF){
	   indeks++;
	   banyak_customer++;
		}
	fclose(pmb);
...
{
	Karakter(109);
	printf("\nNo | Nama                       | Alamat                         |   Kode  Laptop       |  Jumlah           |");
	printf("\n=============================================================================================================");
	for(int j=0;j<banyak_customer;j++)
	{
		printf ("\n%-2d | %-25s  | %-30s | %-20s |  %-16d |",j+1, customer[j].nama, customer[j].alamat, customer[j].barangdibeli,customer[j].jmlhbrg );
	}
	printf("\n");
	printf("\npress ENTER to continue ");
	getch();
...
void beliLaptop()
{
	...
	fflush(stdin);
	printf("Masukkan kode barang yang dibeli : "); scanf("%s", &customer[indeks].barangdibeli);
	printf("Masukkan jumlah barang : "); scanf("%d", &customer[indeks].jmlhbrg);
